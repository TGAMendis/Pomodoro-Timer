const Logo = () => /*#__PURE__*/React.createElement("img", { className: "icon icons8-Tomato", src: "data:image/png;base64,", width: "50", height: "50" });

class App extends React.Component {
  constructor(props) {
    super(props);

    this.state = {
      breakTime: 250,
      workTime: 1500,
      seconds: 1500,
      timerId: false,
      active: 'workTime' };


    this.playStop = this.playStop.bind(this);
    this.updateTime = this.updateTime.bind(this);
  }

  // 
  updateTime() {
    this.setState(function (prevState, props) {
      const currentState = Object.assign(prevState);
      const stillActive = prevState.seconds - 1 > 0;
      const nextTimer = prevState.active === 'workTime' ? 'breakTime' : 'workTime';

      currentState.seconds = stillActive ? currentState.seconds - 1 : currentState[nextTimer];
      currentState.active = stillActive ? currentState.active : nextTimer;
      if (this.timerID) {
        currentState.timerId = this.timerID;
      }
      return currentState;
    });
  }

  // 
  playStop() {
    if (this.state.timerId) {
      clearInterval(this.state.timerId);
      return this.setState({
        seconds: this.state.workTime,
        timerId: false,
        active: 'workTime' });

    }

    this.timerID = setInterval(() => this.updateTime(), 1000);
  }
  // 
  updateLength(timer, e) {
    if (this.state.timerId) {
      return false;
    }

    const state = Object.assign({}, this.state);
    state[timer] = e.target.value * 60;
    state.seconds = timer === 'workTime' ? e.target.value * 60 : state.seconds;
    this.setState(state);
  }
  render() {
    const buttonString = this.state.timerId ? 'Stop' : 'Start';
    return /*#__PURE__*/(
      React.createElement("div", { className: "app" }, /*#__PURE__*/
      React.createElement(Logo, null), /*#__PURE__*/
      React.createElement(Time, { active: this.state.active, seconds: this.state.seconds }), /*#__PURE__*/
      React.createElement(Button, { action: this.playStop }, buttonString), /*#__PURE__*/
      React.createElement(Option, { value: this.state.workTime, timer: "workTime", updateLength: this.updateLength.bind(this) }, "Minutes of work"), /*#__PURE__*/
      React.createElement(Option, { value: this.state.breakTime, timer: "breakTime", updateLength: this.updateLength.bind(this) }, "Minutes of break")));


  }}


class Option extends React.Component {
  onChange(e) {
    e.preventDefault();
    this.props.updateLength(this.props.timer, e);
  }

  convertToMinutes(seconds) {
    return Math.floor(seconds / 60);
  }

  render() {
    return /*#__PURE__*/(
      React.createElement("label", { className: "input-group" },
      this.props.children, /*#__PURE__*/
      React.createElement("input", { className: "input-group__input", type: "number", min: "1", step: "1", placeholder: "Insert minutes", onChange: this.onChange.bind(this), value: this.convertToMinutes(this.props.value) })));


  }}


const Button = props => /*#__PURE__*/React.createElement("button", { className: "btn", onClick: props.action }, props.children);

class Time extends React.Component {
  twoDigits(num) {
    return num > 9 ? "" + num : "0" + num;
  }

  convertToHhMmSs(seconds) {
    const h = this.twoDigits(Math.floor(seconds / 3600));
    const m = this.twoDigits(Math.floor(seconds % 3600 / 60));
    const s = this.twoDigits(Math.floor(seconds % 3600 % 60));
    return `${h}:${m}:${s}`;
  }

  render() {
    var remainingTime = this.convertToHhMmSs(this.props.seconds);
    var activeTimer = this.props.active === 'workTime' ? 'It\'s time to work!' : 'Take a little break';

    return /*#__PURE__*/(
      React.createElement("div", { className: "timer" }, /*#__PURE__*/
      React.createElement("p", { className: "timer__description" }, activeTimer), /*#__PURE__*/
      React.createElement("p", { className: "timer__time" }, remainingTime)));


  }}


ReactDOM.render( /*#__PURE__*/React.createElement(App, null), document.getElementById('root'));